...
class MirrorStatus:
    ...
    STATUS_FAILED = "Gagal 🚫. membersih Unduh..."
            ...
            if INDEX > COUNT:
                ...
                if download.status() != MirrorStatus.STATUS_ARCHIVING and download.status() != MirrorStatus.STATUS_EXTRACTING:
                    ...
                    if download.status() == MirrorStatus.STATUS_CLONING:
                        msg += f"\n<b>↳🚦Cloned:</b> <code>{get_readable_file_size(download.processed_bytes())}</code> ke <code>{download.size()}</code>"
                    elif download.status() == MirrorStatus.STATUS_UPLOADING:
                        msg += f"\n<b>↳📤Diunggah :</b> <code>{get_readable_file_size(download.processed_bytes())}</code> ke <code>{download.size()}</code>"
                    else:
                        msg += f"\n<b>↳📥Diunduh :</b> <code>{get_readable_file_size(download.processed_bytes())}</code> ke <code>{download.size()}</code>"
                    msg += f"\n<b>↳⚡️Spd:</b> <code>{download.speed()}</code>" \
                            f" | <b>⏰Est:</b> <code>{download.eta()}</code> "
                    ...
                    try:
                        ...
                        pass
                    msg += f"\n<b>↳❌Batal:</b> <code>/{BotCommands.CancelMirror} {download.gid()}</code>"
                msg += "\n\n"
                if STATUS_LIMIT is not None:
